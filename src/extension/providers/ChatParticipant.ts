...
export class ChatParticipant {
   ...
   *
   * IMPLEMENTATION: Uses existing AgentLoop for full autonomous behavior
   */
  public async handleRequest(
    ...
    try {
      ...
      // Track what content has already been sent to avoid duplication
      let sentContentLength = 0;
      let lastThinkingUpdate = 0; // Track last thinking update time
      const THINKING_UPDATE_INTERVAL = 2000; // Only update thinking every 2 seconds

      // Execute using existing AgentLoop - full autonomous behavior!
      // FOLLOWS THE EXACT SAME PATTERN AS WebviewManager.executeAgentLoop()
      await this.agentLoop.execute(
        messages,
        (update: AgentLoopUpdate) => {
          // Handle different update types - SAME AS WebviewManager
          switch (update.type) {
            case 'chunk':
              // Send ONLY NEW content (not accumulated content)
              if (update.content) {
              ...
              // Show thinking as progress (brief preview only, rate-limited)
              if (update.thinking && !update.content) {
                const now = Date.now();
                // Only update thinking display every 2 seconds to avoid spam
        ...
        tools,
        'gpt-oss:120b-cloud' // Use default model
      );

      // Add action button to open full ForgeAI panel
      stream.button({
